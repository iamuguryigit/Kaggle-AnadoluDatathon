...
def data_transform(df):
    ...
    #Since there are less missing data and imputing techniques does not improve the result, just fill random number or drop from the data.
    ...
    for columns in ["GELIR","COCUK_SAYISI","MUSTERI_SEGMENTI"]:
        df["columns"] = df["columns"].astype(int)
    
    # PARTICIPATION_AGE 
    df["BASLANGIC_YILI"] = df["BASLANGIC_TARIHI"].str[:4].astype(int) -df["DOGUM_TARIHI"]
    ...
    # data type conversion for catboost.
...
def determine_threshold(train_pred,val_pred,y_train,y_val):
    ...
    for i in np.linspace(0.4,0.6,20):
        ...
        train_score = met.f1_score(y_train,iter_train_result)
        val_score = met.f1_score(y_val,iter_val_result)
...
def plotLearningCurve(est, X_train,y_train,X_test,y_test,y_lower = 0.75, y_upper = 1.0):
     ...
     for i in range(2000,len(X_train),2000):
         ...
         # y_values_train.append(mod.cross_val_score(est,X_train,y_train,scoring = "f1_score",cv=5).mean())
